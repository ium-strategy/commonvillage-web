 ...
 *  커먼빌리지 — 공고·지역 정보 파일
 *
 *  담당자가 직접 고치는 파일은 이것 하나뿐입니다.
 *  고치는 방법은 docs/2_직접-고칠-수-있는-것.md 를 보세요.
 *
 *  고치기 전에 이 파일을 복사해서 백업해 두세요.
 *  글자만 바꾸고 { } [ ] , ' 같은 기호는 건드리지 마세요.
 *  기호 하나만 깨져도 사이트가 안 나옵니다.
 *
 *  ⚠ 지금 들어 있는 공고와 지역 점수는 전부 화면 확인용 예시입니다.
 *    실제 모집 정보가 아닙니다. 진짜 정보로 바꾸기 전까지
 *    각 화면 맨 위의 검은 안내 띠를 지우지 마세요.
 *
 *  이 파일에 들어 있는 것 (위에서부터 순서대로)
 *    1) SITE        — 카카오톡 채널 주소
 *    2) METRIC_DEFS — 지역 점수 6가지의 이름
 *    3) REGIONS     — 지역 정보와 점수
 *    4) PROGRAMS    — 공고 목록  ← 가장 자주 고치게 될 부분
 *    5) LABELS      — 화면에 나오는 선택지 이름
 * ============================================================ */
...
/* ─────────────────────────────────────────────────────────
 *  3) 지역 정보          ★ 여기부터 지역 목록입니다
 *     지역 하나는 { 로 시작해 }, 로 끝나는 한 덩어리입니다.
 *     m(점수, 설명, 출처, 기준일, 갱신주기) 순서입니다.
 *     모르는 점수는 m(null, '데이터 없음', null, null, '반기') 로.
 * ───────────────────────────────────────────────────────── */
const REGIONS = [
  {
 ...
 */
/* ─────────────────────────────────────────────────────────
 *  4) 공고 목록          ★★ 여기부터 공고입니다 (가장 자주 고치는 곳)
 *
 *     공고 하나는 { 로 시작해 }, 로 끝나는 한 덩어리입니다.
 *     새 공고를 넣을 때는 기존 덩어리를 통째로 복사해서
 *     아래에 붙여넣고 내용만 바꾸는 것이 가장 안전합니다.
 *
 *     날짜는 반드시 '2027-01-10' 형식으로 쓰세요.
 *     id 는 다른 공고와 겹치면 안 됩니다.
 *     마감된 공고는 지우지 마세요. 자동으로 '지난 모집' 탭으로 넘어갑니다.
 * ───────────────────────────────────────────────────────── */
const PROGRAMS = [
  {
...
/* ─────────────────────────────────────────────────────────
 *  5) 화면에 나오는 선택지 이름
 *     따옴표 안의 한글만 바꾸세요. 앞의 영어 이름표는 그대로 두어야 합니다.
 * ───────────────────────────────────────────────────────── */
const LABELS = {
